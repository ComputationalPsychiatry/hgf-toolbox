function r = simModel(inputs, prc_model, prc_pvec, varargin)
...
%                        - 'hgf_binary'        The binary Hierarchical Gaussian Filter (HGF) model
%                                              in the absence of perceptual uncertainty
%                        - 'hgf_binary3l'      The binary Hierarchical Gaussian Filter (HGF) model,
%                                              restricted to 3 levels, no drift and no inputs at
%                                              irregular intervals, in the absence of perceptual
%                                              uncertainty. This is the old version of hgf_binary
%                                              and is only included in the toolbox for backwards
%                                              compatibility.
%                        - 'rw_binary'         Rescorla-Wagner
...
%                        - 'unitsq_sgm'        The unit-square sigmoid binary decision model.
%                                              Compatible with hgf_binary and rw_binary.
%                        - 'softmax_binary'    The binary softmax decision model.
%                                              Compatible with hgf_binary and rw_binary.
%                        - 'gaussian_obs'      Gaussian noise decision model. Compatible with hgf.
...
%                        to the comments in their configuration files (e.g., for hgf_binary to
%                        hgf_binary_config.m). Note that the settings there are only relevant
%                        for fitting these models with the fitModel function.
% OUTPUT:
...
%     structure returned by simModel(...) as its only argument.
% EXAMPLE:
%     sim = simModel(inputs, 'hgf_binary', [NaN 0 1 NaN 1 1 NaN 0 0 1 1 NaN -2.5 -6], 'unitsq_sgm', 2);
