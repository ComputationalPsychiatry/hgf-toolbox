function [traj, infStates] = tapas_ehgf_jget(r, p, varargin)
    ...
    if not(ismember(k-1, r.ign))
        ...
        if l > 2
            ...
            for j = 2:l-1
                ...
                % Mean updates
                mux(k,j) = muxhat(k,j) +1/2 *1/pixhat(k,j) *kax(j-1) *wx(k,j-1) *dax(k,j-1);
                mua(k,j) = muahat(k,j) +1/2 *1/piahat(k,j) *kaa(j-1) *wa(k,j-1) *daa(k,j-1);

                % Ingredients of precision updates which depend on the mean
                % updates
                vvx = t(k) *exp(kax(j-1) *mux(k,j) +omx(j-1));
                vva = t(k) *exp(kaa(j-1) *mua(k,j) +oma(j-1));
                pixmhat = 1/(1/pix(k-1,j-1) +vvx); 
                piamhat = 1/(1/pia(k-1,j-1) +vva); 
                wwx = vvx *pixmhat;
                wwa = vva *piamhat;
                rrx = (vvx -1/pix(k-1,j-1)) *pixmhat;
                rra = (vva -1/pia(k-1,j-1)) *piamhat;
                ddx = (1/pix(k,j-1) +(mux(k,j-1) -muxhat(k,j-1))^2) *pixmhat -1;
                dda = (1/pia(k,j-1) +(mua(k,j-1) -muahat(k,j-1))^2) *piamhat -1;
                
                % Precision update
                pix(k,j) = pixhat(k,j) +max(0, 1/2 *kax(j-1)^2 *wwx(wwx +rrx*ddx));
                pia(k,j) = piahat(k,j) +max(0, 1/2 *kaa(j-1)^2 *wwa(wwa +rra*dda));
        ...
        % Mean updates
        mux(k,l) = muxhat(k,l) +1/2 *1/pixhat(k,l) *kax(l-1) *wx(k,l-1) *dax(k,l-1);
        mua(k,l) = muahat(k,l) +1/2 *1/piahat(k,l) *kaa(l-1) *wa(k,l-1) *daa(k,l-1);

        % Ingredients of precision updates which depend on the mean
        % updates
        vvx = t(k) *exp(kax(l-1) *mux(k,l) +omx(l-1));
        vva = t(k) *exp(kaa(l-1) *mua(k,l) +oma(l-1));
        pixmhat = 1/(1/pix(k-1,l-1) +vvx); 
        piamhat = 1/(1/pia(k-1,l-1) +vva); 
        wwx = vvx *pixmhat;
        wwa = vva *piamhat;
        rrx = (vvx -1/pix(k-1,l-1)) *pixmhat;
        rra = (vva -1/pia(k-1,l-1)) *piamhat;
        ddx = (1/pix(k,l-1) +(mux(k,l-1) -muxhat(k,l-1))^2) *pixmhat -1;
        dda = (1/pia(k,l-1) +(mua(k,l-1) -muahat(k,l-1))^2) *piamhat -1;
        
        % Precision update
        pix(k,l) = pixhat(k,l) +max(0, 1/2 *kax(l-1)^2 *wwx*(wwx +rrx*ddx));
        pia(k,l) = piahat(k,l) +max(0, 1/2 *kaa(l-1)^2 *wwa*(wwa +rra*dda));
