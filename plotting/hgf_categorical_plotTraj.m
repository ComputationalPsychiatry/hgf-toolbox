function hgf_categorical_plotTraj(r)
% Plots trajectories estimated by fitModel for the hgf_categorical perceptual model
% Usage:  est = fitModel(responses, inputs); hgf_plotTraj(est);

% Check whether we have a configuration structure
if ~isfield(r,'c_prc')
    error('tapas:hgf:ConfigRequired', 'Configuration required: before calling hgf_categorical_plotTraj, hgf_categorical_config has to be called.');
end
