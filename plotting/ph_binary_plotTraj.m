function ph_binary_plotTraj(r)
% Plots trajectories estimated by fitModel for the rw_binary perceptual model
% Usage:  est = fitModel(responses, inputs); rw_binary_plotTraj(est);
...
if ~isempty(find(strcmp(fieldnames(r),'y'))) && ~isempty(r.y)
    ...
    title(['Response y (orange), input u (green), and value v (red) for \alpha_0=', ...
           num2str(r.p_prc.al_0), ', v_0=', num2str(r.p_prc.v_0), ', S=', num2str(r.p_prc.S)], 'FontWeight', 'bold');
    ylabel('y, u, v');
    axis([0 t -0.15 1.15]);
else
    title(['Input u (green) and value v (red) for \alpha_0=', ...
           num2str(r.p_prc.al_0), ', v_0=', num2str(r.p_prc.v_0), ', S=', num2str(r.p_prc.S)], 'FontWeight', 'bold');
    ylabel('u, v');
    axis([0 t -0.1 1.1]);
end
