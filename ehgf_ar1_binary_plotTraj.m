function ehgf_ar1_binary_plotTraj(r)
% Plots the estimated or generated trajectories for the binary HGF perceptual model
% Usage example:  est = fitModel(responses, inputs); hgf_binary_plotTraj(est);
...
if (ploty == true) && ~isempty(find(strcmp(fieldnames(r),'y'))) && ~isempty(r.y)
    if ~isempty(find(strcmp(fieldnames(r),'c_sim'))) && strcmp(r.c_sim.obs_model,'beta_obs')
        y = r.y(:,1);
    else
    ...
    title(['Response y (orange), input u (green), learning rate (fine black), and posterior expectation of input s(\mu_2) ', ...
           '(red) for \phi=', num2str(r.p_prc.phi(2:end)), ', m=', num2str(r.p_prc.m(2:end)), ...
           '\rho=', num2str(r.p_prc.rho(2:end)), ', \kappa=', ...
