function c = unitsq_sgm_mu3_config
...
% f(x) = x^zeta/(x^zeta + (1-x)^zeta) = ussgm(x; zeta),
...
% p(y=1|mu1hat) = ussgm(mu1hat; zeta)
...
% Model filehandle
c.obs_fun = @unitsq_sgm_mu3;

% Handle to function that transforms observation parameters to their native space
% from the space they are estimated in
c.transp_obs_fun = @unitsq_sgm_mu3_transp;
